...
def clear():
    os.system("cls") if name == 'nt' else os.system("clear")


def font(your_font, your_text):
    your_text = your_text.replace(" ", "+")

    if your_font not in all_fonts:
        print("Error, Font not Found!")
        exit()

    api = f"https://artii.herokuapp.com/make?font={your_font}&text={your_text}"
    asciiart = requests.get(api).text

    return asciiart


def fonts_list(*your_text):
    if not your_text:
        your_text = 'Zelow'
    ...
    for fonth in all_fonts:
        clear()
        print("Name of the Font: " + fonth + "\n\n" + font(fonth, your_text))
        input()